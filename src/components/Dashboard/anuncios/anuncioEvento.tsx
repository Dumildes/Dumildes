...
const AnuncioEvento: React.FC = () => {
    ...
    useEffect(() => {
        ...
        CNPApi.get<{ anuncios: Anuncio[] }>('anuncios?&perPage=50')
            .then(res => {
                setLoading(false)
                setAnuncios(res.data.anuncios.filter(anuncio => anuncio.tipo === 'Evento'));
            }).catch(() => {
                setLoading(false)
            })
    }, [])
                        ...
                        <CardContent>
                            <p style={{ display: 'flex', alignItems: 'center', marginBottom: '10px' }}>
                                ...
                                <span style={{
                                    ...
                                    marginTop: '0',
                                    background:
                                        anuncio.status === 'Activo' ? 'rgba(124, 236, 54, 0.757)' :
